...
def em_run_fixcen(conf,
    ...
    for i in range(conf['maxiter']):
        ...
        set_logtau_logdet(gmix_conv, sums)
        ...
        for pixel in pixels:
            ...
            gsum, tlogL = do_scratch_sums_fixcen(pixel, gmix_conv, sums)
    ...
    # we have modified the mixture and not set the norms,
    # and we don't want to set them for the pre-psf mixture
    gmix['norm_set'][:] = 0
...
def em_run_ponly(conf,
    ...
    for i in range(conf['maxiter']):
        ...
        for pixel in pixels:
            ...
            gsum = do_scratch_sums_ponly(pixel, gmix_conv, sums)
